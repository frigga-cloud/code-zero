...
CodeZero puts a commit graph, diffs, blame and a coding agent in one VS Code
panel — and connects them to your organisation's indexed code, so the agent can
answer questions about repositories that are not checked out on this machine.
...
Search **CodeZero** in the Extensions view, or:
...
| **Chat** | Ask CodeZero about your code. The agent works against your open repository *and* your organisation's index. |
...
**Files** — every repository CodeZero has indexed. The one you have open expands
to your real working tree, uncommitted changes and all; the rest expand to their
indexed file list, and those files open too. That is the difference between a
listing and something you can actually read.
...
CodeZero drives the Claude Code you already have, with its own tools attached
and a gate in front of your files.
...
- **indexed** — CodeZero's index knows this repository. It can be searched,
  traced and read from anywhere.
...
including changes you have not committed. A file marked **cloud** is CodeZero's
indexed copy: the last commit it indexed, not anyone's working tree.

Click a repository CodeZero cannot find on disk and it offers to **Locate** it
first, and only then to **Clone** it — a second copy of a repository you already
have is nobody's idea of a good outcome. You always choose where a clone lands.

## Requirements

- **VS Code 1.90.0 or newer.** VS Code itself — the forks are not supported.
- **A Frigga account with CodeZero access.** Without it, sign-in succeeds and
  then nothing else can, so it is worth knowing before you install rather than
  after.
- **A `claude` binary on your PATH**, for agent turns only. CodeZero runs the
  Claude Code you already installed and signed into rather than shipping its
...
| `code0.backendUrl` | The CodeZero backend, which serves plans, tasks and the repository index. Leave empty for the hosted default. |
...
Installed from the Marketplace, VS Code updates CodeZero on its own schedule.
...
- the first line of the **code0** channel in the Output panel
  (`Ctrl`/`Cmd`+`Shift`+`P` → *Output*, then pick **code0** from the dropdown).
  It reports the build and the commit it was cut from.
